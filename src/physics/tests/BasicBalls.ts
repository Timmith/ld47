...
import { World, Body, Vec2 } from '~/vendor/Box2D/Box2D'
...
export default class BasicBalls implements IPhysicsTest{
    ...
    report(){
      return ['simulated ' + this.circleBodies.length + ' balls and a few boxes.',
  ...
  update(dt: number) {
    for (const circleBody of this.circleBodies) {
      ...
      if (p.y < -1) {
        ...
        circleBody.SetPositionXY(detRandPhysics(-1, 1), 1 + detRandPhysics(-0.2, 0.2))
      }
    }
  }
}